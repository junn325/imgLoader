...
function _load_image(e, f, d) {
  ...
  if (history.pushState && pcnt <= prl) {
    ...
    api_call(dispatch_xhr, b, function () {
      load_image_dispatch();
    });
    if (!d) {
...
function scroll_space(a) {
  var b = get_ypos();
  window.scrollBy(0, Math.round(window.innerHeight * 0.75 * (a ? -1 : 1)));
  if (!a && b == get_ypos()) {
...
function update_window_extents() {
  var d = Math.max(700, window.innerWidth - 70);
  var b = xres;
  var c = yres;
  if (b > d) {
    c = Math.round((c * d) / b);
...
function nl(a) {
  document.location +=
    ((document.location + "").indexOf("?") > -1 ? "&" : "?") + "nl=" + a;
  return false;
}
window.onpopstate = function (a) {
  if (a.state != undefined) {
    if (typeof a.state.json != "undefined" && a.state.expire > get_unixtime()) {
      apply_json_state(a.state.json);
    } else {
      _load_image(a.state.page, a.state.imgkey, true);
    }
  }
};
window.onresize = update_window_extents;
setTimeout(function () {
  update_window_extents();
}, 50);
if (history.replaceState) {
  setTimeout(function () {
    history.replaceState(
      { page: startpage, imgkey: startkey },
      document.title,
      document.location
    );
  }, 50);
  ...
  if (b.shiftKey) {
    ...
    switch (a) {
      ...
      case KeyEvent.DOM_VK_NUMPAD5:
        document.getElementById("loadfail").onclick();
        cancelEvent(b);
        break;
    }
  }
