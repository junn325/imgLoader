...
namespace imgL
{
    internal class Processor
    {
        ...
        internal bool LoadInfo()
        {
            ...
            _title       = GetTitle(_site.GetTitle(), _artist);
            _route       = GetInfoPath(_title);
            _info        = _site.GetInfo();
        ...
        private static string GetTitle(string title, string artist)
        {
            ...
            if (artist.Length == 0)
            {
                ...
                var i = temp.Length;
                while (Encoding.Unicode.GetByteCount(result) > 255)
                {
                    result = temp[..i--] + "...";
                }
            }
            else
            {
                ...
                if (artist.Length > 18) artist = artist[..15] + "...";
                while (Encoding.Unicode.GetByteCount(result) > 255)
                {
                    result = $"{temp[..i--]}... ({artist})";
                }
            }
        ...
        private string GetInfoPath(string title)
        {
            if (title == null) return null;

            return $@"{Core.Route}\{title}\{Core.Dir.EHNumFromRaw(_number)}.{Core.InfoExt}";
        }
